...
class CompilerOutputContractEvm(TypedDict):
    legacyAssembly: None
    bytecode: Dict[str, str]


class CompilerOutputContract(TypedDict):
    abi: List[Dict]
    evm: CompilerOutputContractEvm
    factoryDependencies: Dict
    hash: str


class CompilerOutput(TypedDict):
    contracts: Dict[str, Dict[str, CompilerOutputContract]]
    errors: List
...
class ZkSyncCompiler(CompilerAPI):
    ...
    def config(self) -> ZkSyncConfig:
        return cast(ZkSyncConfig, self.config_manager.get_config(self.name))
    ...
    def compile(
        ...
        for solc_version, arguments in compiler_arguments.items():
            ...
            logger.debug(f"Compiling using zksolc compiler '{solc_version}'")
            ...
            for contract_path, contracts in output["contracts"].items():
                for contract_name, contract in contracts.items():
                    ...
                    for input_file_path in contract_filepaths:
                        if contract_path in str(input_file_path):
                            break
                    else:
                    ...
                    try:
                        deployment_bytecode = contract["evm"]["bytecode"]["object"]
                    except KeyError:
                        deployment_bytecode = ""

                    # Skip library linking.
                    if "__$" in deployment_bytecode:
                    ...
                    contract_type_obj = {
                        ...
                        "deploymentBytecode": {"bytecode": deployment_bytecode},
                        "runtimeBytecode": {},
                        "abi": contract["abi"],
                    }
                    contract_type = ContractType.parse_obj(contract_type_obj)
    ...
    def get_compiler_input(self, base_path: Path, files: List[Path]) -> CompilerInput:
        return {
            "language": "Solidity",
            "sources": {
                str(get_relative_path(file, base_path)): {"content": file.read_text()}
                for file in files
            },
            "settings": {
